...
import { tokenContext } from '../../context/tokenContext'; // Verify the path
...
export default function Login() {
  ...
  const { setToken } = useContext(tokenContext); // Ensure useContext is used correctly
  ...
  const validationSchema = Yup.object({
    ...
    password: Yup.string().matches(/^[A-Z][a-z0-9]{5,8}$/, 'Password should start with a capital letter').required('Required'),
  });
    ...
    <div className="mx-auto max-w-screen-xl px-4 py-16 sm:px-6 lg:px-8">
      ...
      <form onSubmit={formik.handleSubmit} className="mx-auto mb-0 mt-8 max-w-md space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            type="submit"
            className={`inline-block rounded-lg px-5 py-3 text-sm font-medium text-white ${isLoading ? 'bg-blue-300' : 'bg-blue-500'}`}
